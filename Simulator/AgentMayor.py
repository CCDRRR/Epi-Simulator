...
class AgentMayor(Agent):
            ...
            if infection_rate > 0.3:
                ...
                if district_id in self.mask_policies:
                    ...
                    if self.mask_policies[district_id] <= 0:
                        self.model.mask_policy = False
                        del self.mask_policies[district_id]
                        self.record_policy("lifted", district_id, "mask")

        # print(f"Policy log length: {len(self.policy_log)}")
        # if len(self.policy_log) > 0:
            # print(f"Policy log sample: {self.policy_log[:5]}")

    def calculate_infection_rates(self):
        infection_rates = {district_id: 0 for district_id in self.model.districts.values()}
        for agent in self.model.schedule.agents:
            if isinstance(agent, SIERDAgent):
                district_id = self.model.districts[agent.pos]
                if district_id in infection_rates:
    ...
    def record_policy(self, status, district_id, policy_type):
        self.policy_log.append((self.model.schedule.time, status, district_id, policy_type))

    def export_policy_records(self, filename):
        try:
            df = pd.DataFrame(self.policy_log, columns=["time", "status", "district_id", "policy_type"])
            df.to_csv(filename, index=False)
            print(f"Policy records saved to {filename}")
            print(f"Current working directory: {os.getcwd()}")
            print(f"Policy log saved with {len(self.policy_log)} records.")
        except Exception as e:
            print(f"Failed to save policy records: {e}")